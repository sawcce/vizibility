...
use inkwell::values::{BasicMetadataValueEnum, FunctionValue, AnyValue};
...
impl<'ctx> CodeGen<'ctx> {
    ...
    fn test_program(&self) -> Option<()> {
        ...
        let param_a = i64.const_int(9, false);
        let param_b = i64.const_int(28, false);

        let result = self
            .builder
            .build_call(condition, &[param_a.into(), param_b.into()], "call");

        let val = result.try_as_basic_value().left().unwrap();
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    codegen.test_program().ok_or("Unable to compile")?;
